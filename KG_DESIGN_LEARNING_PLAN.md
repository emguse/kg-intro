...
## Case Expansion: 持ち帰り先と活用イメージ

目的: 機械装置と部品の小さな KG 事例を、自分の業務や組織でどんな用途に広げられるか考えます。
...
| --- | --- | --- |
| 保全、設備管理 | 廃番部品の影響装置を洗い出す | 装置、構成ユニット、部品、後継部品、保全作業 |
| 予備品管理 | どの部品を在庫すべきか判断する | 部品、使用装置、重要度、リードタイム、代替品 |
| 調達、購買 | サプライヤ変更や供給停止の影響を調べる | 部品、サプライヤ、型番、互換品、対象装置 |
| 品質保証 | 不具合部品の使用範囲を追跡する | 不具合、ロット、部品、装置、出荷先、点検履歴 |
| 生産技術 | 設備改造の影響範囲を確認する | ライン、工程、装置、構成ユニット、関連作業 |
| 設計、開発 | 設計変更が既存装置へ及ぼす影響を確認する | 図面、部品、BOM、装置モデル、改訂履歴 |
| FSE | 現地対応時に症状から対象部品、過去事例、推奨作業をたどる | 装置、症状、エラーコード、部品、作業手順、訪問記録 |
| テクニカルサポート | 問い合わせ内容から類似事例、原因候補、回答案を探す | 問い合わせ、症状、原因、対処、装置モデル、FAQ |
| 現場教育 | 新人が装置構成や保全手順をたどって学ぶ | 装置、部品、作業手順、故障モード、写真 |
| 技術継承 | ベテランの暗黙知を検索できる形にする | 症状、原因、対処、装置、部品、作業メモ |

### 事例を広げるときの考え方

最初に広げる方向は、データをたくさん足すことではなく、答えたい問いを 1 つ増やすことです。

例:

- 廃番部品の影響を知りたいなら、`Part -superseded_by-> Part` と `Component -uses_part-> Part` を育てる
- 在庫判断に使いたいなら、`Part` に `lead_time`, `criticality`, `stock_quantity` を足す
- 不具合追跡に使いたいなら、`Failure`, `Lot`, `InspectionRecord` をノード候補にする
- 保全計画に使いたいなら、`MaintenanceTask`, `interval`, `last_performed_at` を設計する
- FSE の現地対応に使いたいなら、`ServiceVisit`, `ErrorCode`, `Symptom`, `WorkProcedure` をつなぐ
- テクニカルサポートに使いたいなら、`SupportCase`, `FAQ`, `Cause`, `Countermeasure` をつなぐ
- 技術継承に使いたいなら、`Symptom`, `Cause`, `Countermeasure` をつなぐ

### 持ち帰りワーク

自分の業務に持ち帰るときは、次の順で小さく考えます。

1. 自分の職場で「探すのに時間がかかる問い」を 3 つ書く
2. その問いに登場する名詞をノード候補にする
3. 名詞同士の動詞を relation 候補にする
4. Excel、図面、CMMS、ERP、BOM、点検記録など、元データの所在を書く
5. まず 10 ノード、10 エッジ程度の手書き KG に落とす
6. その KG で最初の問いに答えられるか確認する

完了条件:

- 機械装置KGの学習内容を、自分の業務領域の問いに置き換えられる
- 部門ごとに増やすべきノード、エッジ、属性の候補を説明できる
- 「まず小さく作るKG」と「将来つなぎたい業務データ」を分けて考えられる
